...
class supplierclass:
    def __init__(self,root):
        ...
        #=====row4============================================
        lbl_adress=Label(self.root,text="Description",font=("goudy old style",15),bg="white").place(x=50,y=200)
    ...
    def create_supplier_table(self):
        try:
            ...
            print(f"Error creating supplier table: {str(e)}")
...
####################======================================================================================
    ...
    def show(self):
        ...
        try:
            ...
            for row in rows:
            # Insert values without the serial number
                self.supplierTable.insert('', END, values=(row[0], row[1], row[2], row[3]))
    ...
    def search(self):
        ...
        try:
            if self.var_searchtxt.get() == "":
                ...
                cur.execute("SELECT invoice, name, contact, desc FROM supplier WHERE invoice=?", (self.var_searchtxt.get(),))
                row = cur.fetchone()
                if row is not None:
                    self.supplierTable.delete(*self.supplierTable.get_children())
                # Insert values without the serial number
